...
function draw() {
  drawCount += 1;
    ...
    // if frameCount < than 1000 frames draw background
    // after 1000 loops draw houses
    if(drawCount < end){
      setBackground();
    }else{
      buildHouse();
...
function setBackground() {
  rectangle[rectangle.length] = { x: random(1, 800), y: random(1, 800) , w: random(1, 80), h:random(1, 80)}
}

function buildHouse() {
  houseHeightCunt += 1;
  // each layer is shifted by certain coordinates,
  // so that the house looks like a house when you look down from above
  houseX += random(0,2);
  houseY += random(3,0);
  // after 29 layers have been laid, 
  // the roof is put in place as last layer and then the new position of the structures will determined by random
